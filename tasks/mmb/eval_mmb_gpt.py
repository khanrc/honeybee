...
# From https://github.com/open-compass/opencompass/blob/main/tools/eval_mmbench.py
import argparse
import json
...
def load(f):
    ...
    def load_jsonl(f):
        with open(f, encoding="utf-8") as f_:
            lines = f_.readlines()
    ...
    while retry:
        ...
        if "Failed to obtain answer via API" in ans:
            ...
            if ret:
                ...
                double_log(f'GPT output includes 0 / >1 letter in "ABCD": {ans}', fout)
                retry -= 1
...
def eval_sub_data(model, sub_data, answer_map, use_gpt_matcher):
    lt = len(sub_data)
    GT, PRED = [], []
    ...
    for i in range(lt):
        if PRED[i]:
            ...
            if use_gpt_matcher:
                ...
                # if the use_gpt_matcher is not used, then randomly select an answer for the case of non-exact-match.
...
# Evaluate Results
def eval_result(eval_file, eval_method, meta_file, use_gpt_matcher):
    rd.seed(2680)
    ...
    cate_map = {i: c for i, c in zip(meta["index"], meta["category"])}  # noqa: C416
    l2_cate_map = {i: c for i, c in zip(meta["index"], meta["l2-category"])}  # noqa: C416
    split_map = {i: c for i, c in zip(meta["index"], meta["split"])}  # noqa: C416
    answer_map = {i: c for i, c in zip(meta["index"], meta["answer"])}  # noqa: C416
    ...
    for i in tqdm(range(lt)):
        ...
        ret = eval_sub_data(model, sub_data, answer_map, use_gpt_matcher)
        result[idx] = ret
        hit += ret
        tot += 1
        ...
        if (i + 1) % 10 == 0:
            double_log(
                (f"Evaluating {eval_file}: {i + 1}/{lt}, " f"Acc: {hit / tot * 100: .2f}%. "), fout
            )
...
def parse_args():
    ...
    parser.add_argument("result", type=str, help="The path to your inference result. ")
    parser.add_argument(
        "--meta",
        type=str,
        default="data/mmbench_dev_20230712.tsv",
        help=("The path to your meta file (dev). " "Downloaded from MMBench website. "),
    )
    parser.add_argument("--use_gpt_matcher", action="store_true")
...
if __name__ == "__main__":
    ...
    fout = open(log_pth, "a")  # noqa: SIM115

    acc, l2, leaf = eval_result(args.result, "openai", args.meta, args.use_gpt_matcher)
