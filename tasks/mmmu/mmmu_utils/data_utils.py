...
def process_single_sample(data):
    ...
    n_option_images = len(o_imgs_paths)
    if n_option_images:  # multiple images in options, used for random selection
        return {'id': data['id'], 'question': question, 'options': data['options'], 'answer': data['answer'],
             'image': None, 'question_type': data['question_type'], "num_option_images": n_option_images}
    else:
        return {'id': data['id'], 'question': question, 'options': data['options'], 'answer': data['answer'],
             'image': data['image_1'], 'question_type': data['question_type'], "num_option_images": n_option_images}
...
# DATA PROCESSING
def construct_prompt(sample, config, option_prompt=""):
    question = sample['question']
    options = eval(sample['options'])
    example = option_prompt
