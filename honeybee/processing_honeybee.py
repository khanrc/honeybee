...
class HoneybeeProcessor(ProcessorMixin):
    ...
    def __init__(self, image_processor=None, tokenizer=None, num_visual_token=65, **kwargs):
        ...
        self.media_info = {"<image>": num_visual_token}

    def __call__(self, text=None, images=None, return_tensors="pt", **kwargs):
        if all(img is None for img in images):  # for the text only case
            images = None
        ...
        if images is not None:
            # get null(zero) image indices to apply attention masking during forward pass.
            is_null_image = [np.all(np.array(image) == 0) for image in images]

            # Resize here
            images = [img.resize((self.size, self.size), 3) for img in images if img is not None]
            # images = [image for image in images if image is not None]
            image_features = self.image_processor(images, return_tensors=return_tensors, **kwargs)

        if text is not None and images is not None:
            encoding["pixel_values"] = image_features.pixel_values
            encoding["is_null_image"] = torch.tensor(is_null_image)
...
class HoneybeeImageProcessor(CLIPImageProcessor):
    pass
...
def _tokenize_prompts_and_batch(prompts, tokens_to_generate, add_BOS, tokenizer, **kwargs):
    ...
    for prompt_tokens, prompt_length in zip(prompts_tokens, prompts_length):
        ...
        # extend left (ref: https://stackoverflow.com/a/19849920)
        prompt_tokens[0:0] = [tokenizer.pad_token_id] * padding_size  # left padding

    # Now we are in a structured format, we can convert to tensors.
    prompts_tokens_tensor = torch.LongTensor(prompts_tokens)
    prompts_length_tensor = torch.LongTensor(prompts_length)
    attention_mask = prompts_tokens_tensor != tokenizer.pad_token_id
...
def _tokenize_prompt(prompt, tokenizer, add_BOS=False, media_info={"<image>": 65}, **kwargs):
    ...
    if all(media_token not in prompt for media_token in media_tokens.keys()):
        enc_chunk = (
            prompt_chunk + tokenizer(prompt, add_special_tokens=False, **kwargs)["input_ids"]
        )
